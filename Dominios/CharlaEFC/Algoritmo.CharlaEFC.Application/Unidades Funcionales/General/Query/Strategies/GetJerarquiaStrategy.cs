...
namespace Algoritmo.CharlaEFC.Application.General.Query
{
    ...
    /// </summary>
    public class GetJerarquiaStrategy : GetEntidadStrategy
    {
        public GetJerarquiaStrategy(IWorkContext workContext, GetEntidadStrategyParams execParams) : base(workContext, execParams)
        {
        }
            ...
            where TInAndReturnItem : class, IEntity
        {
            ...
            var query = repo.Entities
                            .Include(t => (t as Jerarquia).Niveles)
                            .Select(t=> t);
            ...
            where TInAndReturnItem : class, IEntity
        {
            ...
            var repo = WorkContext.GetRepository<TInAndReturnItem>();
            var query = repo.Entities
                    //.Include(t => (t as Jerarquia).Arbol)
                    .Include(t => (t as Jerarquia).Niveles);
                    //.Select(t => (t as Jerarquia).SetEntityType(WorkContext.Services.DTOManager.GetMappedType((t as Jerarquia).TipoEntidadHoja).FirstOrDefault()) as TInAndReturnItem);
