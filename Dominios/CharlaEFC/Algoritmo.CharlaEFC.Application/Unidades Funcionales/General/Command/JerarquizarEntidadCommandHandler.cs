...
{
    #region Validator
    public class JerarquizarEntidadCommandValidator : AbstractValidator<JerarquizarEntidadCommand>
    {
        public JerarquizarEntidadCommandValidator()
        {
    ...
    #endregion
    public class JerarquizarEntidadCommandHandler : BaseCommandHandler<JerarquizarEntidadCommand, JerarquizarEntidadResponse>
    {
        public JerarquizarEntidadCommandHandler(IWorkContext workContext) : base(workContext) { }

        /// <inheritdoc cref="JerarquizarEntidadCommand"/>
        public override async Task<JerarquizarEntidadResponse> HandleDelegate(JerarquizarEntidadCommand command, CancellationToken cancellationToken)
        {
            WorkContext.Services.UnitOfWork.EntitiesVersionController.Disable();
            var response = new JerarquizarEntidadResponse(command.CorrelationId);
            ...
            JerarquizarEntidadRecord record = new(command.ItemPadre.Jerarquia.mapMe<Jerarquia>(),
                                                  command.ItemPadre.mapMe<JerarquiaItem>(),
                                                  entidades);

            await em.RunAsync(new JerarquizarEntidadStrategy(WorkContext), record);
        ...
        private void WorkContext_OnSuccess(object sender, Microservices.Shared.Domain.Services.Events.WorkContextEvents.OnSuccessfulRequestEventArgs e)
        {
            ...
            var response = (JerarquizarEntidadResponse)e.Response;
